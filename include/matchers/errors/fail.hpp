...
class Fail : public MatcherBase<A, void *> {
 public:
  ...
  bool match() { return !this->actual().get_status(); }
};
...
class FailWith : public MatcherBase<A, std::string> {
 public:
  ...
  FailWith(Expectation<A> &expectation, std::string expected)
      : MatcherBase<A, std::string>(expectation, expected) {}

  bool match() {
    return (!this->actual().get_status()) && this->actual().get_message() == this->expected();
  }
};
