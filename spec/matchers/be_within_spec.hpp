...
describe be_within_spec(
  "expect(actual).to_be_within(delta).of(expected)", _blk  {
	it("passes when actual == expected", _blk {
		expect(5.0).to_be_within(0.5).of(5.0)();
	});

	it("passes when actual < (expected + delta)", _blk {
	...
	it("passes with integer arguments that are near each other", _blk {
		expect(1.0001).to_be_within(5).percent_of(1)();
	});	

	it("passes with negative arguments", _blk {
		expect(-1.0001).to_be_within(5).percent_of(-1)();
	});

	// Do failures here

	it("provides a description", _blk {
		auto ex(5.1);
	...
	context("expect(actual).to_be_within(delta).percent_of(expected)", _blk {
		it("passes when actual is within the given percent variance", _blk {
		...
		// TODO: PEDNING
		// it("fails when actual is outside the given percent variance", _blk {
		// 	expect([]{
	...
	context("expect(actual).not_().to_be_within(delta).of(expected)", _blk {
		it("passes when actual < (expected - delta)", _blk {
		...
		// it("fails when actual == expected", _blk {
		// 	expect([]{
		// 		expect(5.51).not_().to_be_within(0.5).of(5.0)();
