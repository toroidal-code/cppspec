...
std::string Equal<A, E>::failure_message_when_negated() {
  std::stringstream ss;
  ss <<  "\n" 
      "expected not " << Util::inspect_object(BaseMatcher<A,E>::get_actual()) << "\n"
      "         got " << Util::inspect_object(BaseMatcher<A,E>::get_expected()) << "\n"
      "\n"
      "Compared using `==`.\n\n";
  return ss.str();
}
...
//   std::stringstream ss;
//   ss << "\nexpected " << std::boolalpha << BaseMatcher<A,bool>::get_expected() << "\n"
